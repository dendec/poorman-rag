...
import (
	...
	mcp_api "github.com/dendec/poorman-rag/internal/api/mcp"
	...
	"github.com/dendec/poorman-rag/internal/services/mcp"
	"github.com/dendec/poorman-rag/internal/services/search"
	"github.com/dendec/poorman-rag/internal/utils"
)

type App struct {
	Handler       *mcp_api.Handler
...
func NewApp(configPath string) (*App, error) {
	...
	searchService := search.NewService(s3Opts, cfg.IndexDir, cfg.Aliases, dim, embedder, utils.StringFromEnvDefault("RAG_SEARCH_TYPE", domain.SearchHybrid))

	searchService.SetRRFConfig(cfg.RRFK, cfg.LimitVector, cfg.LimitFTS, cfg.TopK)

	service := mcp.NewService(searchService)
	handler := mcp_api.NewHandler(service)
...
func initRemoteEmbedder() (domain.Embedder, int, error) {
...
func initLocalEmbedder(cfg *config.Config) (domain.Embedder, int, error) {
...
func handleRequest(ctx context.Context, request events.APIGatewayV2HTTPRequest) (events.APIGatewayV2HTTPResponse, error) {
	...
	if response == nil {
		// This happens for notifications that don't require a response
