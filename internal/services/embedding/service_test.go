...
func TestService(t *testing.T) {
	...
	t.Run("ComputeEmbedding_Success", func(t *testing.T) {
		...
		assert.Equal(t, domain.Embedding(expectedEmbedding), result)
	...
	t.Run("ComputeEmbeddings_Success", func(t *testing.T) {
		...
		assert.Equal(t, domain.Embedding([]float32{0.1, 0.2}), results[0])
		assert.Equal(t, domain.Embedding([]float32{0.3, 0.4}), results[1])
		mockEmbedder.AssertExpectations(t)
	})
	...
	t.Run("ValidateEmbedding_Valid", func(t *testing.T) {
		...
		embedding := domain.Embedding{0.1, 0.2, 0.3}
