...
type SearchService interface {
	...
	GetKnowledgeBases() []domain.KnowledgeBase
}
...
func NewService(opts utils.S3Options, indexDir string, aliases []string, dim int, embedder domain.Embedder, searchType string) *Service {
...
func (s *Service) GetKnowledgeBases() []domain.KnowledgeBase {
	aliases := s.ListAliases()
	kbs := make([]domain.KnowledgeBase, len(aliases))

	for i, alias := range aliases {
		kbs[i] = domain.KnowledgeBase{
			Name:        alias,
